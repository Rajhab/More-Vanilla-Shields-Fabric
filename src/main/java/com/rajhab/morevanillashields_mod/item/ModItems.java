...
public class ModItems {
    ...
    public static final Item REDSTONE_SHIELD = registerItem("redstone_shield",
            new ShieldItem(new Item.Settings().maxDamage(250).group(ItemGroup.COMBAT)){
                ...
                public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
                    if (Screen.hasShiftDown()) {
                        tooltip.add(new TranslatableText("item.moditems.redstone_shield").append("250").append(new TranslatableText("item.moditems.redstone_shield.particles_enabled")).formatted(Formatting.DARK_AQUA));
                    } else {
                        tooltip.add(new TranslatableText("item.moditems.shift").formatted(Formatting.LIGHT_PURPLE));
                    }
